...
from marine_mammal_toolkit.tools._core.config.paths import current_workspace
...
class ConfigDocument:
    """A fully composed, validated, hashable toolkit configuration."""
    ...
    def load(
        ...
        *,
        ...
        source = (
            (Path(workspace_root).expanduser().resolve() / candidate).resolve()
            if workspace_root is not None and not candidate.is_absolute()
            else resolve_config_path(candidate)
        )
    ...
    def resolve_path(self, value: str | Path) -> Path:
        ...
        return (
            ...
            else ((self.workspace_root or project_root()) / candidate).resolve()
        )
