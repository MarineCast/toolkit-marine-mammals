    ...
    if spec.schema is not None:
        for field in spec.schema:
            ...
            if index < 0:
                ...
                errors.append(
                    f"Column {field.name} has type {table.schema.field(index).type}; expected {field.type}"
                )
                schema_valid = False
    if spec.schema is not None:
        for field in spec.schema:
...
def validate_path(path: Path, spec: DatasetSpec) -> ValidationReport:
    ...
    if spec.format in {DatasetFormat.PARQUET, DatasetFormat.GEOPARQUET}:
        try:
            ...
            if not files:
                return ValidationReport(
                    False,
                    str(spec.dataset_id),
                    errors=("Parquet dataset has no files",),
                )
            ...
            for item in parquet_files[1:]:
                other_schema = item.schema_arrow
                if other_schema.names != schema.names or any(
                    other_schema.field(name).type != schema.field(name).type
                    for name in schema.names
                ):
                    errors.append(
                        "Parquet dataset partitions have inconsistent schemas"
                    )
                    schema_valid = False
                    break
            if spec.schema is not None:
            ...
            if spec.schema is not None:
                for field in spec.schema:
                    if not field.nullable and all(
                        field.name in item.schema_arrow.names for item in parquet_files
                    ):
                        null_count = sum(
            ...
            if spec.primary_key:
                ...
                if missing:
                    ...
                    # Read only the key projection. Source history contains a
                    # very large JSON payload column that is irrelevant to the
                    # uniqueness check and previously dominated validation.
                    key_tables = [
                        item.read(columns=list(spec.primary_key))
                        for item in parquet_files
                    ]
                    ...
                    if null_count:
                        errors.append(
                            f"Primary key has {null_count} rows with null values"
                        )
                        key_unique = False
                    duplicate_count = int(keys.duplicated().sum())
                    if duplicate_count:
                        errors.append(
                            f"Primary key has {duplicate_count} duplicate rows"
                        )
                        key_unique = False
            return ValidationReport(
                valid=not errors,
