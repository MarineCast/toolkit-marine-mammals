"""Export Southern Resident killer whale population counts for the app.
...
def _resolve_base_directory(config_path: Path, raw_base_dir: object) -> Path:
    """Resolve the configured base directory against the OrcaCast project root."""
...
def load_population_config(
    ...
    Relative ``base_directory`` values are interpreted relative to the OrcaCast
    project root, matching the rest of the repository's configuration contract.
    """
    ...
    *,
    ...
    if workspace_root is not None:
        ...
        with workspace(workspace_root):
            return export_population_numbers(
    ...
    rows = load_population_rows(source, sheet_name=sheet_name)
